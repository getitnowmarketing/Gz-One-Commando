...
char Diag_Audio_qtr_loopback( Diag_Audio_qtr_loopback_type *data)
{
	...
	else if((devid % 16) == 2)
	{
		...
		rx_profile = & ispeaker_rx_profile;
		dev_name_rx = "speaker_stereo_rx";
	}
...
static int __init snddev_icodec_pw_init(void)
{
	int pm_ret;
	pm_ret = pmic_hsed_enable(PM_HSED_CONTROLLER_1, PM_HSED_ENABLE_ALWAYS);
	printk(KERN_INFO "snddev_icodec_pw_init:HSED1(always) = %d\n", pm_ret);
	return 0;
}
late_initcall(snddev_icodec_pw_init);
