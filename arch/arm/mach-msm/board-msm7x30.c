...
#include "qdsp5v2/yda160_i2c_driver.h"
...
static struct android_usb_product usb_products[] = {
	...
	{
		.product_id	= 0x02EC,
		.num_functions	= ARRAY_SIZE(usb_functions_eut),
		.functions	= usb_functions_eut,
	},
	{
		.product_id	= 0x02ED,
...
static struct android_usb_platform_data android_usb_pdata = {
	...
	.version	= 0x0100,
	.product_name		= "C771",
	.manufacturer_name	= "CASIO COMPUTER CO.,LTD.",
...
static struct i2c_board_info msm_i2c_board_info[] = {
	{
		...
		I2C_BOARD_INFO( YDA160_I2C_DEVICE_NAME , YDA160_I2C_SLAVE_ADDRESS ) ,
		.platform_data = &yda160_pdata,
		.irq = MSM_GPIO_TO_INT(71),
	}
	,{
...
static struct msm_gpio synopex_spi_gpio_config_data[] = {
	
	
	{ GPIO_CFG(92, 0, GPIO_CFG_INPUT , GPIO_CFG_PULL_UP, GPIO_CFG_2MA), "synopex_irq" },
};

static int synopex_hw_setup(struct device *dev)
{
	int rc = 0;

	rc = msm_gpios_request_enable(synopex_spi_gpio_config_data,
								  ARRAY_SIZE(synopex_spi_gpio_config_data));
	printk(KERN_DEBUG "[synopex]%s:%d\n", __func__, rc);

	gpio_set_value(88, 1);

	return rc;
}
static int synopex_hw_powerdown(struct device *dev)
{
	gpio_set_value(88, 0);
...
static int synopex_hw_powerup(struct device *dev)
{
	gpio_set_value(88, 1);
...
static void synopex_hw_teardown(struct device *dev)
{
	printk(KERN_DEBUG "[synopex]%s\n", __func__);
}
static struct synopex_platform_data synopex_pdata = {
	.setup = synopex_hw_setup,
	.powerup = synopex_hw_powerup,
	.powerdown = synopex_hw_powerdown,
	.teardown = synopex_hw_teardown,
	.msm_ts_data = &msm_ts_data,
};

static struct spi_board_info msm_spi_board_info[] __initdata = {
	{
		.modalias	= "msm-touchscreen",
		.mode		= SPI_MODE_1,
		.irq		= MSM_GPIO_TO_INT(92),
		.bus_num	= 0,
		.chip_select	= 3,
		.max_speed_hz = 1000000,
		.platform_data = &synopex_pdata,
	},
};
...
static struct msm_gpio qsd_spi_gpio_config_data[] = {
	...
	{ GPIO_CFG(89, 2, GPIO_CFG_INPUT,  GPIO_CFG_NO_PULL, GPIO_CFG_2MA), "spi_cs3" },
};
...
static int msm_fb_detect_panel(const char *name)
{
    ...
    if (!strncmp(name, "mddi_hitachi_wvga_pt", sizeof("mddi_hitachi_wvga_pt")-1))
        return 0;
...
static int display_common_power(int on)
{
	...
	if (on) {
		...
		rc = gpio_tlmm_config(gpio_lcd_cs, GPIO_CFG_ENABLE);
		if (rc) {
			pr_err("%s: gpio_tlmm_config(%#x)=%d\n",
				       __func__, gpio_lcd_cs, rc);
			return rc;
		}
	...
	mdelay(21);     /* ensure power is stable */
