...
PMIC_USB_AUDIO_TYPE pmic_usb_audio_get_usb_type( bool usb_connect )
{
  bool usb_acc = FALSE;
  int rc = 0;
  PMIC_USB_AUDIO_TYPE rslt = PMIC_USB_AUDIO_TYPE_DISCONNECTED;
  ...
  if( usb_connect != false )
  {
    ...
    if ( usb_acc == true )
    {
      printk(KERN_ERR"[pmic_usb_audio_get_usb_type] connected USB_AUDIO");
      ...
      if ( g_pmic_usb_audio_gpio_stat != PMIC_GPIO_HIGH_VALUE )
      {
        ...
        printk(KERN_ERR"[pmic_usb_audio_get_usb_type] USB_AUD_SW set High Value");
      }
      
      rslt = PMIC_USB_AUDIO_TYPE_AUDIO;
      printk(KERN_ERR"return PMIC_USB_AUDIO_TYPE_AUDIO");
    }
    
    else
    {
      
      if ( g_pmic_usb_audio_gpio_stat != PMIC_GPIO_LOW_VALUE )
      {
        
        gpio_set_value( PMIC_GPIO_AUD_CONN_ID, PMIC_GPIO_LOW_VALUE );
        
        
        g_pmic_usb_audio_gpio_stat = PMIC_GPIO_LOW_VALUE;
        
        printk(KERN_ERR"[pmic_usb_audio_get_usb_type] USB_AUD_SW set Low Value");
      }
      
      rslt = PMIC_USB_AUDIO_TYPE_OTHER;
      printk(KERN_ERR"return PMIC_USB_AUDIO_TYPE_OTHER");
    }
  ...
  else
  {
	...
	usb_vbus_flg = false;
      ...
      rslt = PMIC_USB_AUDIO_TYPE_DISCONNECTED;
      printk(KERN_ERR"return PMIC_USB_AUDIO_TYPE_DISCONNECTED");
  }
...
static void msm_otg_start_peripheral(struct otg_transceiver *xceiv, int on)
{
	...
	rslt = pmic_usb_audio_get_usb_type( on );
	
	
	switch( rslt )
	{
	case PMIC_USB_AUDIO_TYPE_AUDIO:  
		
		hs_report_headphone_usbaudio(USBAUDIO,true);
		
		usb_audio = TRUE;
		break;
	case PMIC_USB_AUDIO_TYPE_DISCONNECTED:  
		if( usb_audio == TRUE )  
		{				
			
			hs_report_headphone_usbaudio(USBAUDIO,false);
			
			usb_audio = FALSE;
		}
		
		break;
	case PMIC_USB_AUDIO_TYPE_OTHER:  
		break;
	default:
		break;
	}
