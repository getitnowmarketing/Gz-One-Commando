...
static void usb_chg_detect(struct work_struct *w)
{
	...
	printk(KERN_DEBUG "%s: msm72k_udc_usb_state=%d, usb_get_chg_type=%d, usb_audio=%d\n",
		__func__, msm72k_udc_usb_state, temp, usb_audio);

	if (temp != USB_CHG_TYPE__SDP || usb_audio == true) {
...
static int msm72k_udc_vbus_session(struct usb_gadget *_gadget, int is_active)
{
	...
	if (!is_active) {
		...
		if (msm72k_udc_connected) {
			...
			cdrom_mode_interrupt();
			printk(KERN_ERR "%s: cdrom_mode_interrupt\n", __func__);
...
static int msm72k_pullup_internal(struct usb_gadget *_gadget, int is_active)
{
  ...
  int rc = 0;
	...
	if (is_active) {
		if((usb_audio == 0) && (usb_vbus_flg == true)){
...
int usb_ioctl_init( )
{
    ...
    alloc_ret = alloc_chrdev_region(&devno, 0, DEVICE_COUNTS, "usb_diag");
    ...
    usb_diag_class = class_create(THIS_MODULE, "usb_diag");
...
static void msm72k_udc_TLBS_timer(unsigned long data)
{
	int	ret;

	printk(KERN_ERR "%s: TL-BS timer is fired!! data=%ld\n", __func__, data);

	ret = comm_usb_request_close(USBMODE_CDROM);
	printk(KERN_DEBUG "%s: comm_usb_request_close(CLOSE) ret=%d\n",
		__func__, ret);
}
...
static int __init init(void)
{
	...
	ret = f_timer_init(EN_MODE_TL_BS, msm72k_udc_TLBS_timer, 0);
	if (ret) {
		goto misc_deregister;
	}
