...
static int get_string(struct usb_composite_dev *cdev,
		void *buf, u16 language, int id, const struct usb_ctrlrequest *ctrl)
{
	struct usb_configuration	*c;
	struct usb_function		*f;
	int				len;
	...
	list_for_each_entry(c, &cdev->configs, list) {
		if(id == 0xee){
			...
			for (i = 0; i < MAX_CONFIG_INTERFACES; i++)
			{
				if(c->interface[i] == NULL)
					break;

				if((strcmp(c->interface[i]->name, "mtp") == 0) &&
					(c->interface[i]->hidden != true))
				{
					break;
				}
			}
			
			if((c->interface[i] != NULL) && (i < MAX_CONFIG_INTERFACES)){
				f = c->interface[i];
				if(f->setup != NULL){
					len = f->setup(f, ctrl);
					if (len > 0)
