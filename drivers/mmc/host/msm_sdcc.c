...
static int diag_mmc_open(struct inode *inode, struct file *file)
{
	return 0;
}

static int diag_mmc_release(struct inode *inode, struct file *file)
{
	return 0;
}

static int
diag_mmc_ioctl(struct inode *inode, struct file *file, unsigned int cmd,
	   unsigned long arg)
{
	int ret = -EINVAL;
...
static struct msm_gpio sdc4_init_cfg_data[] = {
...
static struct msm_gpio sdc4_on_cfg_data[] = {
...
static struct msm_gpio sdc4_off_cfg_data[] = {
...
int diag_mmc_on( void )
{
	int ret = 0;
	struct msmsdcc_host *host = sd_mmc_host;
	struct mmc_host *mmc = sd_mmc;
	struct vreg *vreg_gp10;
	int count = 0;
	
	if( host == NULL || mmc == NULL )
	{
		printk(KERN_ERR "%s: msmsdcc_host or msm_host is NULL.\n", __func__);
		return (-1);
	}
	
	if( host->clks_on == 1 )
	{
		printk(KERN_ERR "%s: Now mmc (ON)\n", __func__);
		return 0;
	}

	
	ret = msm_gpios_request_enable( sdc4_init_cfg_data, ARRAY_SIZE(sdc4_init_cfg_data) );
	if( ret != 0 )
	{
		printk(KERN_ERR "%s: Failed to enable GPIO sdc4_init\n", __func__);
		return (-1);
	}

	for( count = 0; count < ARRAY_SIZE(sdc4_init_cfg_data); count++ )
	{
		ret = gpio_direction_output( GPIO_PIN(sdc4_init_cfg_data[0].gpio_cfg), 0 );
		if( ret != 0 )
		{
	...
	msm_gpios_disable_free(sdc4_on_cfg_data, ARRAY_SIZE(sdc4_on_cfg_data));

	ret = msm_gpios_request_enable( sdc4_on_cfg_data, ARRAY_SIZE(sdc4_on_cfg_data) );
	if( ret != 0 )
	{
		printk(KERN_ERR "%s: Failed to enable GPIO sdc4_on\n", __func__);
		return (-1);
	}
	...
	if (ret)
	{
		printk(KERN_ERR "%s: Failed to clk_enable: pclk\n", __func__);
		clk_put(host->pclk);
		return (-1);
	}
	...
	if( ret != 0 )
	{
		printk(KERN_ERR "%s: Failed to clk_enable: clk\n", __func__);
		clk_put(host->clk);
		clk_disable(host->pclk);
		clk_put(host->pclk);
		return (-1);
	...
	while( count > 1000 )
	{
		ret = mmc_send_op_cond( mmc, 0, NULL );
	...
	if( ret != 0 )
	{
		printk(KERN_ERR "%s: Failed to mmc_send_op_cond: ret(%d)\n", __func__, ret);
	}
	
	return ret;
}

int diag_mmc_off( void )
{
	int ret = 0;
	struct vreg *vreg_gp10;
	struct msmsdcc_host *host = sd_mmc_host;
	...
	msm_gpios_disable_free(sdc4_off_cfg_data, ARRAY_SIZE(sdc4_off_cfg_data));

	ret = msm_gpios_request_enable( sdc4_off_cfg_data, ARRAY_SIZE(sdc4_off_cfg_data) );
	if( ret != 0 )
	{
		printk(KERN_ERR "%s: Failed to enable GPIO sdc4_init\n", __func__);
		return (-1);
	...
	for( count = 0; count < ARRAY_SIZE(sdc4_off_cfg_data); count++ )
	{
		ret = gpio_direction_output( GPIO_PIN(sdc4_off_cfg_data[0].gpio_cfg), 0 );
		if( ret != 0 )
		{
			printk(KERN_ERR "%s: Failed to turn on GPIO (%d) output\n", __func__, count + 58 );
			return (-1);
...
static ssize_t diag_mmc_write(struct file *file, const char __user *data,
                                   size_t len, loff_t *ppos)
{
	...
	printk( KERN_INFO ">>  diag_mmc_write: mmc (%s) ret=(%d)\n", req ? "ON":"OFF", ret );
	
	return ((ssize_t)ret);
}

static struct file_operations diag_mmc_fops = {
	.owner		= THIS_MODULE,
	.write		= diag_mmc_write,
	.open		= diag_mmc_open,
	.release	= diag_mmc_release,
	.ioctl		= diag_mmc_ioctl,
};

static struct miscdevice diag_mmc_device = {
...
{
	misc_deregister(&diag_mmc_device);
