...
struct completion mdp4_overlay_completion;
...
void mdp4_overlay0_done_mddi()
{
	...
	if (pending_pipe)
        ...
        complete(&mdp4_overlay_completion);
...
void mdp4_mddi_dma_busy_wait(struct msm_fb_data_type *mfd, struct mdp4_overlay_pipe *pipe)
{
	unsigned long flag;
	if (pipe == NULL) 
		return;
